...
- Expected results should be clear and measurable
  - Each step should be written as a recommended verification question that describes how to validate the expected behavior.
...
`
  ...
  userPrompt += `\nGenerate test cases covering positive scenarios, negative scenarios, edge cases, and any authorization or non-functional requirements as applicable. For each step, provide a recommended approach by phrasing the action as a question that validates the expected behavior. Return only the JSON response.`
